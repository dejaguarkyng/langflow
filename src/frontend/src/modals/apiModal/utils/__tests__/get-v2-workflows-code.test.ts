...
describe("v2 workflows generators (shared behavior)", () => {
  ...
  it.each(allGenerators)(
    "%s: sync reads the answer from the top-level output_text, not a component-id loop",
    (_name, gen) => {
      const sync = gen(baseOptions).steps[0].code;
      expect(sync).toContain("output_text");
...
describe("getV2WorkflowsCurlCode", () => {
  ...
  it("sync peek shows the flat shape: output_text + session_id, content but no metadata/component_id field", () => {
